...
(() => {
  ...
  function renderBracket() {
    ...
    const tabs = [['all','All rounds'], ...BRACKET_STAGES.map(s => [s,s])];
    ...
    root.innerHTML = stages.map(stage => {
      const ms = matches.filter(m => m.stage === stage).sort((a,b) => Number(a.number)-Number(b.number));
      return `<div class="round-col mobile-active"><h3>${escapeHtml(stage)}</h3>${ms.map(m => matchCard(m)).join('')}</div>`;
    }).join('');
  }
